...
const FORMULATION_SYSTEM = [
  ...
  'If the message lists multiple topics, skill areas, technologies, book types, or requirements (e.g. long job specs, reading lists, or several unrelated themes), you MUST return 3 strings that cover different facets—never collapse everything into one vague query.',
  'Split broad asks into complementary angles: each major theme the user named should get keywords in at least one of the three strings.',
  'The PRIMARY information need is always the latest user message. When it names a specific concept, product, error, or technical term, your queries must center on that wording; do not substitute unrelated topics from earlier turns.',
...
export async function formulateSearchQueries(
  ...
  const prompt = [
    ...
    'If the primary question is long or covers several subjects, output 3 queries that split those subjects.',
    '',
